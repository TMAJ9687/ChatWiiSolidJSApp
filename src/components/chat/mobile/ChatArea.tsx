...
const ChatArea: Component<ChatAreaProps> = (props) => {
  ...
  const [blockingMessage, setBlockingMessage] = createSignal<string | null>(
    null
  );
  ...
  createEffect(() => {
    ...
    const channel = supabase
      .channel(
        `chat-area-blocks-${props.currentUser!.id}-${props.selectedUser!.id}`
      )
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "blocks",
          filter: `or(and(blocker_id.eq.${
            props.currentUser!.id
          },blocked_id.eq.${props.selectedUser!.id}),and(blocker_id.eq.${
            props.selectedUser!.id
          },blocked_id.eq.${props.currentUser!.id}))`,
        },
        async () => {
  ...
  const handleImageSelect = async (event: Event) => {
    ...
    if (file) {
      ...
      const canSend = await photoTrackingService.canSendPhoto(
        props.currentUser.id,
        props.currentUser.role
      );
      if (!canSend.canSend) {
        alert(
          canSend.message || `Daily photo limit reached (${canSend.limit}).`
        );
        input.value = "";
        return;
      }
  ...
  const handleVoiceRecordSend = async (recording: VoiceRecording) => {
    ...
    try {
      ...
      const voiceUrl = await imageService.uploadFile(
        recording.blob,
        props.currentUser.id
      );

      // Create voice data
      const voiceData = voiceService.createVoiceData(
        voiceUrl,
        recording.duration,
        recording.blob.size
      );
  ...
  const handleClearConversation = async (clearType: "all" | "mine") => {
    ...
    try {
      if (clearType === "all") {
        ...
        await conversationService.clearMyMessages(
          props.currentUser.id,
          props.selectedUser.id
        );
      }
  ...
  return (
    <div class="flex-1 flex flex-col bg-neutral-50 dark:bg-neutral-900 max-w-full min-w-0 h-full">
        ...
        fallback={
          <div class="flex-1 flex items-center justify-center p-6">
            <div class="text-center">
              <h3 class="text-lg sm:text-xl font-medium text-text-600 dark:text-text-400 mb-2">
                Select a user to start chatting
              </h3>
              <p class="text-sm sm:text-base text-text-400 dark:text-text-600">
      ...
      >
        ...
        <div class="h-14 sm:h-16 bg-white dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700 px-3 sm:px-4 flex items-center justify-between">
          <div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            <div
              class={`rounded-full p-0.5 ${
                props.selectedUser?.gender === "male"
            ...
            >
              <img
                src={
                  props.selectedUser?.avatar ||
                  `/avatars/standard/${props.selectedUser?.gender}.png`
                }
            ...
            <div class="min-w-0 flex-1">
              <div class="font-medium text-text-1000 dark:text-text-0 mb-0.5 sm:mb-1 truncate text-sm sm:text-base">
              ...
              <div class="flex items-center gap-1 sm:gap-2">
                <img
                  ...
                  class="w-4 h-2.5 sm:w-5 sm:h-3 object-cover"
                  onError={(e) => {
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="p-1.5 sm:p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors"
              aria-label="Close chat"
            >
              <FiX class="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 dark:text-gray-400" />
            </button>
            <ChatOptionsMenu
        ...
        <div class="bg-white dark:bg-neutral-800 border-t border-neutral-200 dark:border-neutral-700 p-3 sm:p-4">
          <div class="flex items-end gap-2">
            {/* Image Upload Button */}
            <label class="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors cursor-pointer">
              <FiImage class="w-4 h-4 sm:w-5 sm:h-5 text-text-600 dark:text-text-400" />
              <input
                type="file"
                accept="image/*"
                class="hidden"
            ...
            <div class="flex-1 relative">
              <input
                ...
                class="w-full pl-9 pr-20 sm:pl-10 sm:pr-24 py-2.5 sm:py-2 bg-neutral-100 dark:bg-neutral-700 rounded-lg
                       text-sm sm:text-base text-text-1000 dark:text-text-0 placeholder:text-text-400 dark:placeholder:text-text-600
                       focus:outline-none focus:ring-2 focus:ring-secondary-500
                       disabled:opacity-50 disabled:cursor-not-allowed
                       min-h-[42px] sm:min-h-[40px]"
              />
              ...
              <button
                ...
                disabled={isBlocked() || isBlockedBy()}
                class="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-neutral-200 dark:hover:bg-neutral-600 transition-colors disabled:opacity-50"
                aria-label="Add emoji"
              >
                <FiSmile class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-text-400 dark:text-text-600" />
              </button>

              {/* Voice message button - far right inside input */}
              <div
                class="absolute right-10 sm:right-12 top-1/2 -translate-y-1/2"
                title={
                  canSendVoice() ? "Send voice message" : "VIP feature only"
                }
              >
                <button
                ...
                >
                  <FiMic class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-text-400 dark:text-text-600" />
                </button>
              </div>

              {/* Character count */}
              <span
                class={`absolute right-2.5 sm:right-3 top-1/2 -translate-y-1/2 text-xs ${getCharCountColor()}`}
              >
            ...
            <button
              ...
              class={`p-2.5 sm:p-2 rounded-lg transition-colors min-w-[44px] sm:min-w-[40px] h-[44px] sm:h-[40px] flex items-center justify-center
                      ${
                        message().length > 0 &&
                        !sending() &&
                        !isBlocked() &&
            ...
            >
              <FiSend class="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
          </div>
        </div>
      </Show>
