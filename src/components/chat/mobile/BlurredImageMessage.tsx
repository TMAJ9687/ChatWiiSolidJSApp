...
export default function BlurredImageMessage(props: BlurredImageMessageProps) {
  ...
  return (
    <div class={`relative max-w-full overflow-hidden ${props.className || ""}`}>
      ...
      >
        ...
        {/* Control buttons - positioned within container bounds */}
        <Show when={imageLoaded()}>
          <div class="absolute top-1 right-1 z-[100]">
            <button
              onClick={toggleReveal}
              class={`
                p-1.5 rounded-full transition-all duration-200 shadow-lg
                ${
                  isRevealed()
