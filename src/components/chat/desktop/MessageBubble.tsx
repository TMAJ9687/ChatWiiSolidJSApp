...
const MessageBubble: Component<MessageBubbleProps> = (props) => {
  ...
  const handleTranslationToggle = (translationResult: TranslationResponse | null) => {
  ...
  return (
    <div
      class={`flex gap-2 mb-3 ${props.isOwn ? "justify-end" : "justify-start"}`}
    >


      <div 
        class={`max-w-[70%] ${props.isOwn ? "order-1" : ""}`}
      ...
      >
        ...
        <div class="relative">
          <div
            class={`${props.message.type === 'image' ? '' : 'px-4 py-2'} rounded-2xl ${
              props.isOwn
          ...
          >
            <Show when={props.message.type === 'text'}>
              <p class="text-sm break-words">{props.message.content}</p>
            </Show>
            ...
            <Show when={props.message.type === 'voice' && props.message.voiceData}>
              <VoicePlayer
                voiceData={{
          ...
          <Show when={showActions() && props.currentUserId}>
            <div class={`absolute top-0 ${props.isOwn ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'} flex items-center gap-1 px-2`}>
